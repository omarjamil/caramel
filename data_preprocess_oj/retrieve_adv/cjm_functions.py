...
def extract_fields_for_advective_tendencies(date,vt,roseid,name_str,analysis_time,region):
    ...
    tmppath_nc_incr = tmppath_nc+'advect_incr/'
    try:
        os.makedirs(tmppath_nc)
    except OSError:
        pass
    try:
        os.makedirs(tmppath_nc_incr)
    except OSError:
        pass
    ...
    outcome=mean_subdomains(flux_theta[:,:,60:300,60:300],date,vt,name_str,analysis_time,region,'99181',3,tmppath_nc_incr)
    outcome=mean_subdomains(flux_qtotal[:,:,60:300,60:300],date,vt,name_str,analysis_time,region,'99182',3,tmppath_nc_incr)
    return outcome;
