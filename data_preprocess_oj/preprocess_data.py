...
def check_files_exist(region: str, date: datetime, subdomain: int, stash: int):
    ...
    location='/net/spice/project/radiation/ML/CRM/data/u-bj775/{0}/stash_{1}/'.format(region, str(stash).zfill(5))
...
def combine_files_per_subdomain(region: str, date: datetime, subdomain: int, stash: int):
    ...
    datestr=date.strftime("%Y%m%d")
    location='/net/spice/project/radiation/ML/CRM/data/u-bj775/{0}/stash_{1}/'.format(region, str(stash).zfill(5))
    out_location='/project/spice/radiation/ML/CRM/data/u-bj775/{0}/concat_stash_{1}/'.format(region, str(stash).zfill(5))
...
def combine_day_tseries(start_date: datetime, end_date: datetime, region: str, subdomain: int, stash: int):
    ...
    location='/project/spice/radiation/ML/CRM/data/u-bj775/{0}/concat_stash_{1}/'.format(region, str(stash).zfill(5))
...
def main_combine_files(region: str, stashes: list):
    # day = sys.argv[1]
    for day in range(1,32):
...
if __name__ == "__main__":
    ...
    stashes = [4,24,1202,1205,1207,1208,1235,2201,2207,2205,3217,3225,3226,3234,3236,3245,4203,4204,9217,30405,30406,30461,16222]
    stashes = [99181, 99182]
    if argument == '1':
