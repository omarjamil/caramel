...
def t_tendency(region: str, subdomain: int, t_stash: int, tadv_stash: int):
    """
    ...
    """
    t_phys_location='/project/spice/radiation/ML/CRM/data/u-bj775/{0}/tend_t_{1}/'.format(region, str(t_stash).zfill(5))
    ...
    t_location='/project/spice/radiation/ML/CRM/data/u-bj775/{0}/concat_stash_{1}/'.format(region, str(t_stash).zfill(5))
    tadv_location='/project/spice/radiation/ML/CRM/data/u-bj775/{0}/concat_stash_{1}/'.format(region, str(tadv_stash).zfill(5))
    d='30_days'
    t_file="{0}_{1}_km1p5_ra1m_30x30_subdomain_{2}_{3}.nc".format(d,region,str(subdomain).zfill(3),str(t_stash).zfill(5))
    tadv_file="{0}_{1}_km1p5_ra1m_30x30_subdomain_{2}_{3}.nc".format(d,region,str(subdomain).zfill(3),str(tadv_stash).zfill(5))
    ...
    outfile="{0}_{1}_km1p5_ra1m_30x30_subdomain_{2}_t_phys_{0}.nc".format(d,region,str(subdomain).zfill(3),str(t_stash).zfill(5))
    print("Saving T tendency ... {0}".format(t_phys_location+outfile))
    iris.fileformats.netcdf.save(new_cube,t_phys_location+outfile)
    
def q_tendency(region: str, subdomain: int, q_stash: int, qadv_stash: int):
    """
    Physics tendency for q quantities
    specific humidity only to start
    Should combine QCL+QCF+Q_v+Q_graupel=Q_p
    ...
    driven change to the quantity
    """
    q_phys_location='/project/spice/radiation/ML/CRM/data/u-bj775/{0}/tend_q_{1}/'.format(region, str(q_stash).zfill(5))
    try:
        os.makedirs(q_phys_location)
    except OSError:
        pass
    
    q_location='/project/spice/radiation/ML/CRM/data/u-bj775/{0}/concat_stash_{1}/'.format(region, str(q_stash).zfill(5))
    qadv_location='/project/spice/radiation/ML/CRM/data/u-bj775/{0}/concat_stash_{1}/'.format(region, str(qadv_stash).zfill(5))
    d='30_days'
    q_file="{0}_{1}_km1p5_ra1m_30x30_subdomain_{2}_{3}.nc".format(d,region,str(subdomain).zfill(3),str(q_stash).zfill(5))
    qadv_file="{0}_{1}_km1p5_ra1m_30x30_subdomain_{2}_{3}.nc".format(d,region,str(subdomain).zfill(3),str(qadv_stash).zfill(5))
    ...
    q_array = q_dat.data[:]
    qadv_array = qadv_dat.data[:]
    q_diff = array_diff(q_array, 1)
    q_phys = q_diff - qadv_array[1:]
    
    # Save the data after normalising
    # Normalise on the fly? Allows different normalisations for training
    #q_phys_norm = normalise_data(q_phys)
    ...
    outfile="{0}_{1}_km1p5_ra1m_30x30_subdomain_{2}_q_phys_{3}.nc".format(d,region,str(subdomain).zfill(3),str(q_stash).zfill(5))
...
def q_tendency_qdot(region: str, subdomain: int):
    ...
    driven change to the quantity
    """
    q_phys_location='/project/spice/radiation/ML/CRM/data/u-bj775/{0}/tend_qphys_{1}/'.format(region, "dot")
    try:
        os.makedirs(q_phys_location)
    except OSError:
        pass
    ...
    # model_lev_coord = iris.coords.DimCoord(q_dat.coord('model_level_number').points,standard_name="model_level_number")
    # new_cube = iris.cube.Cube(q_phys,long_name="q_phys",dim_coords_and_dims=[(time_coord,0),(model_lev_coord,1)])
    # outfile="{0}_{1}_km1p5_ra1m_30x30_subdomain_{2}_q_phys.nc".format(d,region,str(subdomain).zfill(3))
    # print("Saving q tendency ... {0}".format(q_phys_location+outfile))
    # iris.fileformats.netcdf.save(new_cube,q_phys_location+outfile)
...
def main_Q():
    ...
    for subdomain in range(64):
        q_tendency(region,subdomain,q_stash,qadv_stash)
