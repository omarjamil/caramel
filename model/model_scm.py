...
def create_train_scm_model(BATCH: int, EPOCHS: int, train_data: list, test_data: list, model_name: str="scm", region: str="50S69W"):
    """
    """
    global model
    global train_loss
    ...
    # Q and T model
    train_ds = tf.data.Dataset.from_tensor_slices((train_data[0], train_data[1])).shuffle(10000).batch(BATCH)
    test_ds = tf.data.Dataset.from_tensor_slices((test_data[0], test_data[1])).batch(BATCH)
...
if __name__ == "__main__":
    ...
    model_name = "scm"
    region="50S69W"
    model_data_in_train, model_data_out_train, model_data_in_test, model_data_out_test = data_io.scm_model_data(region)
    train_data = []
    test_data = []
    create_train_scm_model(BATCH, EPOCHS, train_data, test_data, model_name=model_name, region=region)