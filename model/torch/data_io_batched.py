...
class ConcatDataset(torch.utils.data.Dataset):
    ...
    def __inverse_transform(self, var, mean, std):
        ...
        feature_max = self.fmax
        feature_min = self.fmin
        invscaled = var.sub(min).div(feature_max-feature_min)
...
class ConcatDatasetY(torch.utils.data.Dataset):
    def __init__(self, dat_type, nlevs, dataset_file, normaliser, xmean, xstd, batch_size, samples_frac=1.,data_frac=1., 
                yvars=["qtot_next", "theta_next"], no_norm=False, fmin=-1, fmax=1):
        super().__init__()
        self.dat_type = dat_type
        self.dataset_file = dataset_file
        self.nlevs = nlevs
        self.nn_norm = NormalizersData(normaliser)
        self.xsize = len(xmean)
        self.xmean = xmean
        self.xstd = xstd
        self.ydat = []
        self.ymean = []
    ...
    def __getitem__(self, i):
        ...
        """
        # print("index {0}".format(i))
        y_var_data = self.__get_train_vars__(i)
        x = torch.normal(self.xmean,self.xstd)
        y = []
        if self.dat_type == "train":
            for yv in self.yvars:
                y.append(y_var_data[yv+"_train"])
