...
class ConcatDataset(torch.utils.data.Dataset):
    def __init__(self, dat_type, nlevs, dataset_file, normaliser, samples_frac=1.,data_frac=1., 
        ...
        self.xdata_and_norm = {
                                ...
                                "theta_"+self.dat_type:[self.theta_train[self.v_slc1, self.v_slc2, self.v_slc3], self.nn_norm.t_mean, self.nn_norm.t_stdscale],
                                "theta_adv_"+self.dat_type:[self.theta_adv_train[self.v_slc1, self.v_slc2, self.v_slc3], self.nn_norm.tadv_mean, self.nn_norm.tadv_stdscale],
                                "sw_toa_"+self.dat_type:[self.sw_toa_train[self.v_slc1, self.v_slc2, :], self.nn_norm.sw_toa_mean, self.nn_norm.sw_toa_stdscale],
                                "shf_"+self.dat_type:[self.shf_train[self.v_slc1, self.v_slc2, :], self.nn_norm.upshf_mean, self.nn_norm.upshf_stdscale],
                                "lhf_"+self.dat_type:[self.lhf_train[self.v_slc1, self.v_slc2, :], self.nn_norm.uplhf_mean, self.nn_norm.uplhf_stdscale],
                                "p_"+self.dat_type:[self.p_train[self.v_slc1, self.v_slc2, self.v_slc3], self.nn_norm.pressure_mean, self.nn_norm.pressure_stdscale],
                                "rho_"+self.dat_type:[self.rho_train[self.v_slc1, self.v_slc2, self.v_slc3], self.nn_norm.rho_mean, self.nn_norm.rho_stdscale],
                                "xwind_"+self.dat_type:[self.xwind_train[self.v_slc1, self.v_slc2, self.v_slc3], self.nn_norm.xwind_mean, self.nn_norm.xwind_stdscale],
                                "ywind_"+self.dat_type:[self.ywind_train[self.v_slc1, self.v_slc2, self.v_slc3], self.nn_norm.ywind_mean, self.nn_norm.ywind_stdscale],
                                "zwind_"+self.dat_type:[self.zwind_train[self.v_slc1, self.v_slc2, self.v_slc3], self.nn_norm.zwind_mean, self.nn_norm.zwind_stdscale],
                                }
        self.ydata_and_norm = {
                                "qphys_"+self.dat_type:[self.qphys_train[self.v_slc1, self.v_slc2, self.v_slc3], self.nn_norm.qphys_mean, self.nn_norm.qphys_stdscale],
                                # "qphys_"+self.dat_type:[self.qphys_train[:self.npoints, :3], self.nn_norm.qphys_mean[0,:3], self.nn_norm.qphys_stdscale[0,:3]],
                                "theta_phys_"+self.dat_type:[self.theta_phys_train[self.v_slc1, self.v_slc2, self.v_slc3], self.nn_norm.tphys_mean, self.nn_norm.tphys_stdscale],
                                ...
                                # "qtot_next_"+self.dat_type:[self.q_tot_train[:self.npoints, :self.nlevs]+self.q_tot_adv_train[:self.npoints, :self.nlevs]+self.qphys_train[:self.npoints, :self.nlevs], self.nn_norm.q_mean, self.nn_norm.q_stdscale],
                                "qtot_next_"+self.dat_type:[self.q_tot_train[self.v_slc1, self.v_slc2, self.v_slc3]+self.q_tot_diff_train[self.v_slc1, self.v_slc2, self.v_slc3], self.nn_norm.q_mean, self.nn_norm.q_stdscale],
                                ...
                                "theta_next_"+self.dat_type:[self.theta_train[self.v_slc1, self.v_slc2, self.v_slc3]+self.theta_diff_train[self.v_slc1, self.v_slc2, self.v_slc3], self.nn_norm.t_mean, self.nn_norm.t_stdscale]
    ...
    def __get_train_vars__(self,indx):
        ...
        for x in self.xvars:
            if self.dat_type == "train":
                ...
                i = x+"_test"
            if self.no_norm:
                x_var_data[i] = torch.from_numpy(self.xdata_and_norm[i][0][isamples, ipoints,:])
            else:
                mean = self.xdata_and_norm[i][1].reshape(-1)[self.norm_slc]
                std = self.xdata_and_norm[i][2].reshape(-1)[self.norm_slc]
                x_var_data[i] = self.__transform(torch.from_numpy(self.xdata_and_norm[i][0][isamples, ipoints,:]), mean, std)
        ...
        for y in self.yvars:
            if self.dat_type == "train":
                ...
                i = y+"_test"
            if self.no_norm:
                y_var_data[i] = torch.from_numpy(self.ydata_and_norm[i][0][isamples, ipoints,:])
            else:
                mean = self.ydata_and_norm[i][1].reshape(-1)[self.norm_slc]
                std = self.ydata_and_norm[i][2].reshape(-1)[self.norm_slc]
                y_var_data[i] = self.__transform(torch.from_numpy(self.ydata_and_norm[i][0][isamples, ipoints,:]), mean, std)
        ...
        for y2 in self.yvars2:
            if self.dat_type == "train":
                ...
                i = y2+"_test"
            if self.no_norm:
                y_var_data2[i] = torch.from_numpy(self.ydata_and_norm[i][0][isamples, ipoints,:])
            else:
                mean = self.ydata_and_norm[i][1].reshape(-1)[self.norm_slc]
                std = self.ydata_and_norm[i][2].reshape(-1)[self.norm_slc]
                y_var_data2[i] = self.__transform(torch.from_numpy(self.ydata_and_norm[i][0][isamples, ipoints,:]), mean, std)
